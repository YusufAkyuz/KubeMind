          ...
          <Table columns={COLUMNS}>
            {visible.map((crb) => (
              <Tr key={crb.name} onClick={() => setSelected(crb)} highlighted={selected?.name === crb.name}>
                <Td className="font-medium text-gray-900 dark:text-slate-100">{crb.name}</Td>
                <Td className="hidden md:table-cell font-mono text-xs text-gray-500 dark:text-slate-400">{crb.roleRefKind}/{crb.roleRefName}</Td>
                <Td className="hidden lg:table-cell text-gray-500 dark:text-slate-400 tabular-nums">{crb.subjects.length}</Td>
                <Td className="text-gray-400 dark:text-slate-500 tabular-nums">{formatAge(crb.creationTimestamp)}</Td>
              </Tr>
            ))}
          </Table>
        </div>
