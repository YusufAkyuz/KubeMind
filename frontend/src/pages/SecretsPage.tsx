      ...
      <DetailDrawer open={!!selected} title={selected?.name ?? ''} subtitle={`Secret · ${selected?.namespace ?? ns}`}
        ...
        {selected && ns && (
          <>
            ...
            {isAdmin && selected.keys.length > 0 && !reveal.data && (
              <div className="pt-3">
                <button
                  ...
                  className="rounded-lg border border-amber-300 bg-amber-50 dark:bg-amber-500/10 px-3 py-2 text-xs font-medium
            ...
            {reveal.isError && (
              <p className="pt-2 text-sm text-red-600 dark:text-red-400">{apiErrorMessage(reveal.error, 'Reveal failed')}</p>
            )}
            {reveal.data && (
              <button
