        ...
        <Table columns={withNamespaceColumn(COLUMNS, showNsColumn)} minWidth="420px">
          {data.map((cm) => (
            <Tr key={`${cm.namespace}/${cm.name}`} onClick={() => setSelected(cm)}
              ...
              <Td className="font-medium text-gray-900 dark:text-slate-100">{cm.name}</Td>
              {showNsColumn && <Td className="text-gray-500 dark:text-slate-400">{cm.namespace}</Td>}
              <Td className="text-gray-500 dark:text-slate-400 tabular-nums">
                {Object.keys(cm.data).length}
                {cm.binaryDataCount > 0 && <span className="text-gray-400 dark:text-slate-500"> (+{cm.binaryDataCount} binary)</span>}
              </Td>
              <Td className="text-gray-400 dark:text-slate-500 tabular-nums">{formatAge(cm.creationTimestamp)}</Td>
            </Tr>
          ))}
        </Table>
      )}
        ...
        {selected && ns && (
          <>
            ...
            {Object.keys(selected.data).length === 0 && (
              <p className="text-sm text-gray-400 dark:text-slate-500">No text data.</p>
            )}
            {Object.entries(selected.data).map(([key, value]) => (
              <div key={key} className="rounded-lg border border-gray-200 dark:border-slate-700 overflow-hidden">
                <p className="px-3 py-1.5 bg-gray-50 dark:bg-slate-800/60 border-b border-gray-200 dark:border-slate-700 text-xs font-mono font-medium text-gray-700 dark:text-slate-300">
                  {key}
                </p>
                <pre className="px-3 py-2 text-xs font-mono text-gray-600 dark:text-slate-400 whitespace-pre-wrap break-all max-h-48 overflow-y-auto">
                  {value}
                </pre>
              </div>
            ))}
