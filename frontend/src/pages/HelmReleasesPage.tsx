    ...
    <Layout>
      ...
      {data && data.length === 0 && !isLoading && (
        <div className="rounded-xl border border-dashed border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-6 py-12 text-center">
          <p className="text-sm text-gray-400 dark:text-slate-500">No Helm releases in this namespace.</p>
        </div>
      )}
        ...
        <Table columns={withNamespaceColumn(COLUMNS, showNsColumn)}>
          {data.map((r) => (
            <Tr key={`${r.namespace}/${r.name}`} onClick={() => { setSelected(r); setTab('values') }}
              ...
              <Td className="text-gray-500 dark:text-slate-400">{r.chart}</Td>
              <Td className="hidden md:table-cell text-gray-500 dark:text-slate-400">{r.appVersion || '—'}</Td>
              <Td className="hidden lg:table-cell text-gray-400 dark:text-slate-500 tabular-nums">{r.revision}</Td>
              <Td className="hidden lg:table-cell text-gray-400 dark:text-slate-500 text-xs">{r.updated}</Td>
            </Tr>
          ))}
        </Table>
      )}
              ...
              {(['values', 'manifest', 'notes'] as const).map((t) => (
                <button
                  ...
                  className={`rounded-md px-2.5 py-1 text-xs font-medium capitalize transition-colors ${
                    tab === t ? 'bg-blue-600 text-white' : 'text-gray-600 dark:text-slate-400 hover:bg-gray-100 dark:hover:bg-slate-700'}`}
                >
                  {t}
                </button>
              ))}
            </div>
            {detailLoading && <p className="text-sm text-gray-400 dark:text-slate-500">Loading…</p>}
                  ...
                  <div className="rounded-md border border-amber-200 dark:border-amber-500/30 bg-amber-50 dark:bg-amber-500/10 px-2.5 py-2 mb-1.5">
                    ...
                    <div className="flex gap-1.5">
                      <select
                        ...
                        className="flex-1 rounded border border-amber-300 bg-white dark:bg-slate-900 px-2 py-1 text-xs
                ...
                {isAdmin && detail.chartRef && (
                  <div className="flex items-center justify-between mt-2">
                    <button
                      ...
                      className="text-xs text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-200 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                    >
                      Revert changes
                    </button>
                    <button
      ...
      <ConfirmDialog
        ...
        message={
          <>
            This will remove the Helm release <span className="font-mono font-medium text-gray-800 dark:text-slate-200">{selected?.name}</span> and
