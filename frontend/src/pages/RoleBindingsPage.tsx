          ...
          <Table columns={withNamespaceColumn(COLUMNS, showNsColumn)}>
            {visible.map((rb) => (
              <Tr key={`${rb.namespace}/${rb.name}`} onClick={() => setSelected(rb)}
                ...
                <Td className="font-medium text-gray-900">{rb.name}</Td>
                {showNsColumn && <Td className="text-gray-500">{rb.namespace}</Td>}
                <Td className="hidden md:table-cell font-mono text-xs text-gray-500">{rb.roleRefKind}/{rb.roleRefName}</Td>
                <Td className="hidden lg:table-cell text-gray-500 tabular-nums">{rb.subjects.length}</Td>
                <Td className="text-gray-400 tabular-nums">{formatAge(rb.creationTimestamp)}</Td>
              </Tr>
            ))}
          </Table>
        </div>
