        ...
        <Table columns={withNamespaceColumn(COLUMNS, showNsColumn)}>
          {data.map((h) => (
            <Tr key={`${h.namespace}/${h.name}`} onClick={() => setSelected(h)}
              ...
              <Td className="font-medium text-gray-900 dark:text-neutral-100">{h.name}</Td>
              {showNsColumn && <Td className="text-gray-500 dark:text-neutral-400">{h.namespace}</Td>}
              <Td><StatusBadge status={status(h)} /></Td>
              <Td className="font-mono text-xs text-gray-500 dark:text-neutral-400">{h.targetRef}</Td>
              <Td className="tabular-nums text-gray-500 dark:text-neutral-400">
                {h.currentReplicas} / {h.minReplicas}–{h.maxReplicas}
              </Td>
              <Td className="hidden lg:table-cell tabular-nums text-gray-400 dark:text-neutral-500">
                {cpuDisplay(h)}
              </Td>
              <Td className="text-gray-400 dark:text-neutral-500 tabular-nums">{formatAge(h.creationTimestamp)}</Td>
            </Tr>
          ))}
        </Table>
      )}
