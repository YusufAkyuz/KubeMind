  ...
  return (
    <Layout>
      ...
      <div className="rounded-xl border border-gray-200 dark:border-neutral-800 bg-gray-950 overflow-hidden shadow-lg flex flex-col"
        ...
        {/* Toolbar */}
        <div className="flex flex-wrap items-center gap-2 px-4 py-2.5 border-b border-gray-800 bg-gray-900 shrink-0">
          {/* Container selector */}
          ...
          <div className="ml-auto flex items-center gap-2 flex-wrap">
            ...
            <div className="relative">
              <IconSearch className="absolute left-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-500" />
              <input
                type="text"
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder="Search logs…"
                className="h-7 w-40 sm:w-56 rounded-md border border-gray-700 bg-gray-800 pl-7 pr-2 text-xs
                           text-gray-200 placeholder-gray-600 focus:outline-none focus:ring-1 focus:ring-blue-500"
              />
              {search && (
                <button
                  onClick={() => setSearch('')}
            ...
            <button
              onClick={clearLogs}
              className="h-7 px-2.5 rounded-md border border-gray-700 text-xs text-gray-400
            ...
            <button
              onClick={reconnect}
              className="h-7 px-2.5 rounded-md border border-gray-700 text-xs text-gray-400
                         hover:bg-gray-800 hover:text-gray-200 transition-colors flex items-center gap-1"
            >
        ...
        <div
          ...
          className="flex-1 overflow-y-auto font-mono text-xs leading-5 text-gray-300 px-4 py-3"
        >
          {filtered.length === 0 && connState === 'connected' && (
            <p className="text-gray-600 italic">{search ? 'No matching lines.' : 'No output yet.'}</p>
          )}
          {filtered.map((line, idx) => (
            <div
              key={line.id}
              className={`flex gap-3 hover:bg-gray-900 -mx-4 px-4 py-px rounded
                          ${search && line.text.toLowerCase().includes(searchLower) ? 'bg-yellow-950/40' : ''}`}
            >
              <span className="select-none text-gray-600 w-10 shrink-0 text-right">
                {idx + 1}
              </span>
        ...
        {/* Status bar */}
        <div className="flex items-center justify-between px-4 py-1.5 border-t border-gray-800 bg-gray-900 shrink-0">
