    ...
    <Layout>
      ...
      {noNamespace && (
        <div className="rounded-xl border border-dashed border-gray-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-6 py-12 text-center">
          <p className="text-sm text-gray-400 dark:text-neutral-500">Select a namespace from the sidebar to view deployments.</p>
        </div>
      )}
        ...
        <Table columns={withNamespaceColumn(COLUMNS, showNsColumn)}>
          {data.map((d) => (
            ...
            >
              ...
              <Td className="tabular-nums text-gray-500 dark:text-neutral-400">{d.readyReplicas}/{d.desiredReplicas}</Td>
              <Td className="hidden md:table-cell text-xs text-gray-400 dark:text-neutral-500">{d.strategy}</Td>
