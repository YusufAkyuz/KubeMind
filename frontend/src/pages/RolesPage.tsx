          ...
          <Table columns={withNamespaceColumn(COLUMNS, showNsColumn)}>
            {visible.map((r) => (
              <Tr key={`${r.namespace}/${r.name}`} onClick={() => setSelected(r)}
                ...
                <Td className="hidden md:table-cell text-gray-500 tabular-nums">{r.rules.length}</Td>
                <Td className="text-gray-400 tabular-nums">{formatAge(r.creationTimestamp)}</Td>
              </Tr>
            ))}
          </Table>
        </div>
