        ...
        <Table columns={COLUMNS}>
          {data.map((pv) => (
            <Tr key={pv.name} onClick={() => setSelected(pv)} highlighted={selected?.name === pv.name}>
              <Td className="font-medium text-gray-900 dark:text-slate-100">{pv.name}</Td>
              <Td><StatusBadge status={PHASE[pv.status] ?? 'Unknown'} /></Td>
              <Td className="text-gray-600 dark:text-slate-400 tabular-nums">{pv.capacity ?? '—'}</Td>
              <Td className="hidden md:table-cell font-mono text-xs text-gray-400 dark:text-slate-500">{pv.claimRef ?? '—'}</Td>
              <Td className="text-gray-400 dark:text-slate-500 tabular-nums">{formatAge(pv.creationTimestamp)}</Td>
            </Tr>
          ))}
        </Table>
      )}
