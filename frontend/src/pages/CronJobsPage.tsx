        ...
        <Table columns={withNamespaceColumn(COLUMNS, showNsColumn)}>
          {data.map((c) => (
            <Tr key={`${c.namespace}/${c.name}`} onClick={() => setSelected(c)}
              ...
              <Td className="font-medium text-gray-900 dark:text-slate-100">{c.name}</Td>
              {showNsColumn && <Td className="text-gray-500 dark:text-slate-400">{c.namespace}</Td>}
              <Td className="font-mono text-xs text-gray-500 dark:text-slate-400">{c.schedule ?? '—'}</Td>
              <Td><StatusBadge status={c.suspended ? 'Pending' : 'Ready'} /></Td>
              <Td className="tabular-nums text-gray-500 dark:text-slate-400">{c.activeJobs}</Td>
              <Td className="hidden lg:table-cell font-mono text-xs text-gray-400 dark:text-slate-500 max-w-[240px] truncate">
                {c.image ?? '—'}
              </Td>
              <Td className="text-gray-400 dark:text-slate-500 tabular-nums">{formatAge(c.creationTimestamp)}</Td>
            </Tr>
          ))}
        </Table>
      )}
