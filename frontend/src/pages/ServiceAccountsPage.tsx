          ...
          <Table columns={withNamespaceColumn(COLUMNS, showNsColumn)}>
            {visible.map((sa) => (
              <Tr key={`${sa.namespace}/${sa.name}`} onClick={() => setSelected(sa)}
                ...
                <Td className="font-medium text-gray-900 dark:text-slate-100">{sa.name}</Td>
                {showNsColumn && <Td className="text-gray-500 dark:text-slate-400">{sa.namespace}</Td>}
                <Td className="hidden md:table-cell text-gray-500 dark:text-slate-400 tabular-nums">
                  {sa.secretCount}{sa.imagePullSecretCount > 0 && ` (+${sa.imagePullSecretCount} pull)`}
                </Td>
                <Td className="hidden lg:table-cell text-gray-500 dark:text-slate-400">{sa.automountToken === false ? 'Disabled' : 'Enabled'}</Td>
                <Td className="text-gray-400 dark:text-slate-500 tabular-nums">{formatAge(sa.creationTimestamp)}</Td>
              </Tr>
            ))}
          </Table>
        </div>
