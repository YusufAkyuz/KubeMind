          ...
          {data.map((ing) => {
            ...
            return (
              <Tr key={`${ing.namespace}/${ing.name}`} onClick={() => setSelected(ing)}
                ...
                <Td className="font-medium text-gray-900 dark:text-slate-100">{ing.name}</Td>
                {showNsColumn && <Td className="text-gray-500 dark:text-slate-400">{ing.namespace}</Td>}
                <Td className="text-gray-500 dark:text-slate-400">{ing.className ?? '—'}</Td>
                <Td className="text-gray-600 dark:text-slate-400 max-w-xs truncate">{hosts || '—'}</Td>
                <Td className="text-gray-400 dark:text-slate-500 tabular-nums">{formatAge(ing.creationTimestamp)}</Td>
              </Tr>
            )
          })}
        </Table>
        ...
        {selected && ns && (
          <>
            ...
            {selected.rules.length === 0 ? (
              <p className="text-sm text-gray-400 dark:text-slate-500">No rules.</p>
            ) : selected.rules.map((r, i) => (
              <div key={i} className="rounded-lg border border-gray-200 dark:border-slate-700 px-3 py-2 text-xs font-mono space-y-0.5">
                <p className="text-gray-800 dark:text-slate-200">{r.host}<span className="text-gray-400 dark:text-slate-500">{r.path}</span></p>
                <p className="text-gray-500 dark:text-slate-400">→ {r.backend}</p>
              </div>
            ))}
          </>
        )}
