          ...
          <Table columns={COLUMNS}>
            {visible.map((cr) => (
              <Tr key={cr.name} onClick={() => setSelected(cr)} highlighted={selected?.name === cr.name}>
                <Td className="font-medium text-gray-900 dark:text-slate-100">{cr.name}</Td>
                <Td className="hidden md:table-cell text-gray-500 dark:text-slate-400 tabular-nums">{cr.rules.length}</Td>
                <Td className="text-gray-400 dark:text-slate-500 tabular-nums">{formatAge(cr.creationTimestamp)}</Td>
              </Tr>
            ))}
          </Table>
        </div>
