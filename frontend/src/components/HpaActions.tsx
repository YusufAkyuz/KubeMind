      ...
      <Modal open={open} title={`Scale ${resourceName}`} onClose={() => setOpen(false)}>
        <div className="space-y-4">
          ...
          <div className="flex gap-4">
            <div>
              <label className="block text-xs text-gray-500 mb-1.5">Min replicas</label>
              <input
                type="number"
                min={1}
                max={500}
                ...
                className="w-24 rounded-md border border-gray-300 px-3 py-2 text-sm tabular-nums
                           focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1.5">Max replicas</label>
              <input
                type="number"
                min={1}
                max={500}
          ...
          {invalid && (
            <p className="text-xs text-red-600">Max replicas must be ≥ min replicas.</p>
          )}
