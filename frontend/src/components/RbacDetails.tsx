...
export function RbacRules({ rules }: { rules: RbacRule[] }) {
  return (
    <>
      ...
      {rules.length === 0 && <p className="text-sm text-gray-400 dark:text-neutral-500">No rules.</p>}
      <div className="space-y-2">
        {rules.map((r, i) => (
          <div key={i} className="rounded-lg border border-gray-200 dark:border-neutral-700 p-3 space-y-1 text-xs">
...
function RuleLine({ label, values, fallback = '—', mono = true, accent = false }: {
  ...
  return (
    <div className="flex gap-2">
      <span className="w-28 shrink-0 text-gray-400 dark:text-neutral-500">{label}</span>
      <span className={`break-all ${mono ? 'font-mono' : ''} ${accent ? 'text-blue-700 dark:text-blue-400' : 'text-gray-700 dark:text-neutral-300'}`}>
...
export function RbacSubjects({ subjects, roleRefKind, roleRefName }: {
  ...
  return (
    <>
      ...
      <div className="rounded-lg border border-gray-200 dark:border-neutral-700 p-3 text-xs">
        <span className="text-gray-400 dark:text-neutral-500">{roleRefKind ?? '—'}</span>{' '}
        <span className="font-mono text-gray-800 dark:text-neutral-200">{roleRefName ?? '—'}</span>
      </div>

      <DrawerSection title={`Subjects (${subjects.length})`} />
      {subjects.length === 0 && <p className="text-sm text-gray-400 dark:text-neutral-500">No subjects.</p>}
      <div className="space-y-2">
        {subjects.map((s, i) => (
          <div key={i} className="rounded-lg border border-gray-200 dark:border-neutral-700 p-3 text-xs flex gap-2">
            <span className="text-gray-400 dark:text-neutral-500 shrink-0">{s.kind}</span>
            <span className="font-mono text-gray-800 dark:text-neutral-200 break-all">
