...
export function SystemManagedToggle({ checked, onChange, hiddenCount }: Props) {
  ...
  return (
    <label className="flex items-center gap-2 text-xs text-gray-500 dark:text-neutral-400 select-none cursor-pointer">
      <input
        type="checkbox"
        checked={checked}
        onChange={(e) => onChange(e.target.checked)}
        className="rounded border-gray-300 dark:border-neutral-600 text-blue-600 dark:text-blue-400 focus:ring-blue-500"
      />
