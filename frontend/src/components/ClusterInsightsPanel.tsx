  ...
  return (
    <div className="mb-4 rounded-lg border border-gray-200 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 border-b border-gray-200">
        <p className="text-sm font-medium text-gray-900">Cluster Insights</p>
        {data.lastUpdated && (
          <p className="text-xs text-gray-400">updated {formatAge(data.lastUpdated)} ago</p>
        )}
      </div>
      ...
      <div className="px-4 py-3 space-y-3">
        <div className="flex flex-wrap gap-x-6 gap-y-1 text-sm text-gray-700">
          <span>{data.nodeCount} node{data.nodeCount === 1 ? '' : 's'}
            {data.nodeVersions.length > 0 && <span className="text-gray-400"> ({data.nodeVersions.join(', ')})</span>}
          </span>
          <span>{data.namespaceCount} namespace{data.namespaceCount === 1 ? '' : 's'}</span>
          <span>{data.podCount} pod{data.podCount === 1 ? '' : 's'}</span>
          <span className={data.unhealthyPodCount > 0 ? 'text-amber-700 font-medium' : 'text-gray-700'}>
        ...
        {data.recentChanges.length > 0 && (
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Recent changes</p>
            <ul className="space-y-0.5">
              {data.recentChanges.slice(0, 5).map((c, i) => (
                <li key={i} className="text-xs text-gray-500 font-mono">
                  {c.action} <span className="text-gray-700">{c.resourceRef}</span> ({c.result})
                </li>
              ))}
            </ul>
          </div>
