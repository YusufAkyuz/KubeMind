...
export function ClusterInsightsPanel({ clusterId }: Props) {
  ...
  return (
    <div className="mb-4 rounded-lg border border-gray-200 dark:border-neutral-700 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-neutral-800/60 border-b border-gray-200 dark:border-neutral-700">
        <p className="text-sm font-medium text-gray-900 dark:text-neutral-100">Cluster Insights</p>
        {data.lastUpdated && (
          <p className="text-xs text-gray-400 dark:text-neutral-500">updated {formatAge(data.lastUpdated)} ago</p>
        )}
      </div>

      <div className="px-4 py-3 space-y-3">
        <div className="flex flex-wrap gap-x-6 gap-y-1 text-sm text-gray-700 dark:text-neutral-300">
          <span>{data.nodeCount} node{data.nodeCount === 1 ? '' : 's'}
            {data.nodeVersions.length > 0 && <span className="text-gray-400 dark:text-neutral-500"> ({data.nodeVersions.join(', ')})</span>}
          </span>
          <span>{data.namespaceCount} namespace{data.namespaceCount === 1 ? '' : 's'}</span>
          <span>{data.podCount} pod{data.podCount === 1 ? '' : 's'}</span>
          <span className={data.unhealthyPodCount > 0 ? 'text-amber-700 dark:text-amber-400 font-medium' : 'text-gray-700 dark:text-neutral-300'}>
          ...
          <div>
            ...
            <ul className="space-y-0.5">
              {data.recentChanges.slice(0, 5).map((c, i) => (
                <li key={i} className="text-xs text-gray-500 dark:text-neutral-400 font-mono">
                  {c.action} <span className="text-gray-700 dark:text-neutral-300">{c.resourceRef}</span> ({c.result})
                </li>
              ))}
            </ul>
          </div>
        ...
        <p className="pt-2 border-t border-gray-100 dark:border-neutral-800 text-xs text-gray-400 dark:text-neutral-500">
